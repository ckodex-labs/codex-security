...
const MODEL_PRICING_NANODOLLARS: Readonly<Record<string, ModelPricing>> = {
  "gpt-5.1-codex-mini": [250, 25, 250, 2_000],
...
export class ScanCostTracker {
  ...
  async #readSessions(): Promise<void> {
    ...
    const included = includedThreadIds(this.#sessions, this.#threadId);
    const usage = aggregateSessionUsage(this.#sessions, included);
...
function includedThreadIds(
  ...
  while (changed) {
    ...
    for (const session of sessions.values()) {
      if (includeChildThread(included, session)) changed = true;
    }
  }
  return included;
}

function includeChildThread(
  included: Set<string>,
  session: SessionUsage,
): boolean {
  if (session.threadId === null || session.parentThreadId === null)
    return false;
  if (!included.has(session.parentThreadId) || included.has(session.threadId)) {
    return false;
  }
...
function aggregateSessionUsage(
  ...
  for (const session of sessions.values()) {
    if (session.threadId === null || !included.has(session.threadId)) continue;
    if (session.usage !== null) usage = addTokenUsage(usage, session.usage);
  }
  return usage;
}
...
function readSessionEvent(line: string, session: SessionUsage): void {
  ...
  if (event["type"] === "session_meta") {
    readSessionMetadata(payload, session);
    return;
  }
  readTokenCountEvent(event["type"], payload, session);
}
...
function readSessionMetadata(
  ...
  if (typeof payload["id"] === "string") session.threadId = payload["id"];
